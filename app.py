...
# ==========================================
# 2. NEET 2026 DETAILED SYLLABUS DATABASE (NO CHANGES)
# ==========================================
NEET_SYLLABUS = {
    "Physics": {
...
# ==========================================
# 4. VISIONARY MODEL SELECTION (Dropdown + Auto-Pilot)
# ==========================================

# 1. Available Models Dictionary (Name -> API Key ID)
# Future models are added for "Visionary Architecture" - if selected, code falls back gracefully.
MODEL_OPTIONS = {
    "🚀 Auto-Pilot (Best Available)": "auto",
    "🥇 Gemini 2.0 Thinking (Experimental)": "models/gemini-2.0-flash-thinking-exp-01-21",
    "🥈 Gemini 1.5 Pro (Reasoning)": "models/gemini-1.5-pro",
    "🥉 Gemini 1.5 Flash (Speed)": "models/gemini-1.5-flash",
    "🔮 Gemini 2.0 Pro (Future)": "models/gemini-2.0-pro",
    "🔮 Gemini 2.5 Pro (Future)": "models/gemini-2.5-pro",
    "🔮 Gemini 3.0 Pro (Future)": "models/gemini-3.0-pro"
}

# 2. Sidebar Dropdown
st.sidebar.markdown("---")
st.sidebar.header("🧠 Neural Engine")
selected_model_label = st.sidebar.selectbox(
    "Select Intelligence Level:",
    options=list(MODEL_OPTIONS.keys()),
    index=0  # Default to Auto-Pilot
)
selected_api_model = MODEL_OPTIONS[selected_model_label]

# 3. Intelligent Model Selector Logic
def get_valid_model(user_choice):
    try:
        # Step A: Get list of models actually available currently from Google
        available_models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
        
        # Step B: Logic Flow
        
        # Case 1: Auto-Pilot (Fail-Safe Ladder Logic)
        if user_choice == "auto":
            if "models/gemini-2.0-flash-thinking-exp-01-21" in available_models:
                return "models/gemini-2.0-flash-thinking-exp-01-21"
            elif "models/gemini-2.0-flash-thinking-exp" in available_models:
        ...
        # Case 2: Specific Model Selected by User
        else:
            # Check if the selected future/specific model actually exists in the API list
            if user_choice in available_models:
                return user_choice
            else:
                # If user selects a "Future" model that doesn't exist yet, fallback to Pro
                st.sidebar.warning(f"⚠️ {user_choice} not yet released by Google. Falling back to 1.5 Pro.")
                return "models/gemini-1.5-pro"
...
MODEL_NAME = get_valid_model(selected_api_model)
# Optional: Verify in sidebar which model is actually running
# st.sidebar.caption(f"Active: `{MODEL_NAME}`") 
...
# ==========================================
# 7. THE SYSTEM PROMPT (MERGED WITH SYLLABUS CONTEXT)
# ==========================================
